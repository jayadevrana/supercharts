...
export type ScanRowStatus = 'ok' | 'insufficient_data' | 'unavailable';
...
export function runScan(candlesBySymbol: Map<string, readonly Candle[]>, req: ScanRequest): ScanResult {
  ...
  for (const [symbol, raw] of candlesBySymbol) {
    ...
    // Metric columns — computed with scan-owned instance ids.
    setIndicatorMetadata([...METRIC_SPECS, ...req.screen.indicatorSpecs]);
