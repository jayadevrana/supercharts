...
## Working agreement (for Codex loop)
...
- 🚀 **Sites publish setup — static SuperCharts website deployed via Sites.** Created the Sites project `supercharts-terminal` (`appgprj_6a33785d3dd88191b2a159299f40af7a`) and added `.openai/hosting.json`. Added `scripts/package-sites.mjs`, which wraps the verified Next static build output into the Sites artifact shape (`dist/server/index.js` + `dist/client/**` + `dist/.openai/hosting.json`) without touching the Fastify API, SQLite data, WebSocket gateway, live alerts, or Telegram config. The hosted artifact serves the marketing/pricing/legal pages and the client-side `/terminal` shell; `/api/*` intentionally returns a transparent 503 because the live Node API is not part of this static Sites deployment. Verified in-session: `pnpm --filter @supercharts/web build` green (existing `lib/format.ts` unused `venue` warning only); `node scripts/package-sites.mjs` produced `.sites-artifacts/supercharts-sites.tgz`; tar listing contains the required Sites paths; local route-mapped preview returned `/` 200 333067, `/pricing` 200 54630, `/terminal` 200 48395, `/legal/terms` 200 20710; sampled terminal JS/CSS assets returned 200. Playwright MCP browser failed to initialize Chrome (`Target page, context or browser has been closed`), so verification used HTTP/content/asset checks instead of a screenshot.
