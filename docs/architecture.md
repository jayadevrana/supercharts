...
- `features/terminal/` — **all terminal UI lives here** (37 files):
  ...
  - **Rails/bars**: `terminal-top-bar.tsx` (Script/Backtest/Import/OANDA/Webhook/Broadcast/MT5 buttons), `left-rail.tsx` (drawing tools), `right-rail.tsx` (tabs: Ind · Data · Layers · News · Heat · P&L · Logs…), `replay-bar.tsx`.
  - **Dialogs**: `alerts-dialog`, `backtest-dialog`, `strategy-builder-dialog`, `signal-builder-dialog`, `indicators-dialog` (browser: favorites/recent/alias search/DnD), `import-csv-dialog`, `oanda-connect-dialog`, `mt5-connect-dialog`, `broadcast-dialog`, `webhooks-dialog`, `layout-picker`.
